...
function SidebarItem({ href, icon, text, isActive = false, isCollapsed = false }: SidebarItemProps) {
  return (
    <Link 
      href={href}
      className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3 px-4'} py-3 rounded-md ${
        isActive 
    ...
    >
      ...
      {!isCollapsed && <div className="text-sm font-medium">{text}</div>}
    </Link>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex h-[calc(100vh-4rem)]">
        {/* Sidebar */}
        <div className={`${isCollapsed ? 'w-16' : 'w-64'} flex-shrink-0 bg-white shadow-md overflow-y-auto transition-all duration-300 ease-in-out`}>
                ...
                {savedTables.map(table => (
                  <div key={table.name} className={`${isCollapsed ? 'px-0 text-center' : 'px-3'} mb-1 relative group`}>
                    <Link 
                      ...
                      className={`flex items-center ${isCollapsed ? 'justify-center py-3' : 'px-4 py-2'} text-sm font-medium rounded-md ${
                    ...
                    >
                      <svg className={`${isCollapsed ? '' : 'mr-3'} h-5 w-5`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      {!isCollapsed && (
                        ...
                        >
                          <svg className="h-4 w-4 text-gray-500 hover:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
